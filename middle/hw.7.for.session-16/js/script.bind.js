...
function AuthModule(loginDafaultData) {
	...
	const showPasswordString = function () {
	...
	this.setLocalStorageDafaultData = function ({email, pass}) {
		localStorage.setItem('inputEmail', email);
		localStorage.setItem('password', pass);
	};

	this.initComponent = function () {
		submit.addEventListener("click", checkData.bind(this)); // пример с bind
		showString.addEventListener("click", showPasswordString.bind(this)); // пример с bind
		btnBack.addEventListener("click", returnBack.bind(this)); // пример с bind
	};
};
